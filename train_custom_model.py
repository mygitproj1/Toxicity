...
print(f"Manual Training Loop executing on: {str(device).upper()}")
print("-" * 27)

# 1. Load and Prepare Data (Same as before)
print("1. Loading Data & Tokenizing...")
...
print("3. Initializing Custom Transformer...")
...
# Optimize model specifically for Intel hardware
model, optimizer = ipex.optimize(model, optimizer=optimizer)

# 5. The Manual Training Loop
EPOCHS = 3
print(f"\n🚀 Starting {EPOCHS} Epochs of Manual Training...")

# Set up the MLflow Experiment
os.environ["MLFLOW_EXPERIMENT_NAME"] = "Custom_Transformer_Architecture"

# Start the MLflow tracker
with mlflow.start_run():
    # Log our architecture hyperparameters so we remember them later
    ...
    for epoch in range(EPOCHS):
        ...
        # ---> THE MAGIC LINE: Send the loss metric to MLflow! <---
        mlflow.log_metric("train_loss", avg_train_loss, step=epoch)

print("\nTraining Complete! Saving custom architecture weights...")
